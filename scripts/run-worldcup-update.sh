#!/bin/bash
#
# launchd가 매일 오전 8시·오후 8시(KST) 호출하는 월드컵 트래커 자동 갱신 래퍼.
...
#
# catch-up: plist는 StartCalendarInterval(매일 08:00, 20:00) + RunAtLoad(부팅/로그인)를 둘 다 둔다.
#           하루 2개 슬롯(AM=08:00~, PM=20:00~)으로 나눠, 슬롯당 1회만 실행한다.
#           자는 중 슬롯 시각이 지났으면 깨어날 때 밀린 실행 1회를 따라잡고,
#           '슬롯 기준 이미 실행' 가드로 같은 슬롯의 중복을 막는다.
#
...
NOW_HM="$(date '+%H%M')"
MORNING_HM=800       # 오전 슬롯 시작 08:00
EVENING_HM=2000      # 오후 슬롯 시작 20:00

# 현재 시각이 속한 갱신 슬롯 결정 (아침 이전이면 빈 값 → 대기)
if [ "$((10#$NOW_HM))" -ge "$EVENING_HM" ]; then
  SLOT="PM"
elif [ "$((10#$NOW_HM))" -ge "$MORNING_HM" ]; then
  SLOT="AM"
else
  SLOT=""
fi
...
if [ "${FORCE:-0}" != "1" ]; then
  if [ -z "$SLOT" ]; then
    echo "[$TS] 오전 슬롯(08:00) 이전 — 스킵하고 대기" >> "$LOG_FILE"
    exit 0
  fi
  if [ -f "$STATE_FILE" ] && [ "$(cat "$STATE_FILE" 2>/dev/null)" = "$SLOT_KEY" ]; then
    echo "[$TS] 이번 슬롯($SLOT_KEY) 이미 실행됨 — 스킵" >> "$LOG_FILE"
    exit 0
  fi
fi
