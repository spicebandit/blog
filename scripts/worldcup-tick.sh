...
2) 대상 .md 파일을 갱신한다. HTML/CSS 구조(.wc-* 클래스, <style> 블록, 브래킷 골격)는 절대 바꾸지 말고, 데이터 값만 바꾼다:
   ...
   - 조 3위 순위표(class="wc-third")를 갱신한다: 12개 각 조의 3위 팀을 모아 승점→득실 순으로 재정렬하고, 순위·조·팀·경기·승점·득실 값을 각 조 순위표와 일치시킨다. 상위 8팀(=각 조 3위 중 32강 진출권) 행에 class="adv", 대한민국 행에 class="kr"을 둔다. 표 행 수(12개)와 HTML 구조는 유지.
