---
title: "Paperclip 입문 — AI 에이전트로 '회사'를 만든다 [1편]"
description: "오픈소스 Paperclip으로 AI 에이전트 팀을 회사처럼 조직하는 법. npx 한 줄 설치, 조직도·예산·거버넌스 개념, Claude Code 연결까지 — 에이전트 '한 명'에서 '조직'으로 넘어가는 첫 가이드."
pubDate: 2026-07-06T08:00:00+09:00
category: ai
tags: ["paperclip", "ai-agent", "멀티에이전트", "에이전트 오케스트레이션"]
draft: true
---

AI 에이전트를 하나 잘 쓰는 것과, 여러 개를 **조직으로 굴리는 것**은 완전히 다른 문제다. 오픈소스 도구 **Paperclip**은 이 두 번째 문제를 정면으로 다룬다 — 에이전트들에게 직함과 보고라인을 주고, 월 예산을 걸고, 승인 없이는 전략을 못 바꾸게 묶는, 말 그대로 "AI 회사"를 만드는 플랫폼이다. 설치는 `npx paperclipai onboard --yes` 한 줄이면 끝나고, MIT 라이선스 오픈소스라 계정도 비용도 필요 없다. [헤르메스 시리즈](/blog/2026-06-28-hermes-agent-nous-research-install-guide/)에서 에이전트 '한 명'을 채용하는 법을 다뤘다면, 이번 2부작은 그 에이전트들로 '회사'를 차리는 이야기다. 1편인 이 글은 개념과 설치·첫 세팅까지, [2편](/blog/2026-07-05-paperclip-ai-newsroom-operations/)은 실제로 열흘간 AI 신문사를 운영하며 얻은 (비싼) 교훈을 다룬다.
