...
## 무료 API가 무너지는 방식 — 미리 알아둘 세 가지

무료 API를 여러 개 붙여 본 뒤에 깨달은 게 있다. **무료 API는 "요금이 오르는" 방식으로 문제를 일으키지 않는다. 조용히 성격이 바뀌는 방식으로 일으킨다.** 패턴이 대체로 셋이다.

**첫째, 무료 티어가 없어지는 게 아니라 쪼그라든다.** 어느 날 서비스가 종료되면 차라리 알아채기 쉽다. 실제로 더 흔한 건 한도가 조용히 줄거나, 예전엔 되던 엔드포인트가 유료 플랜 전용으로 옮겨 가는 경우다. 내 코드는 그대로인데 어느 날부터 특정 필드가 비어서 온다. 그래서 **응답에서 쓰는 필드를 한곳에 모아 두고, 비었을 때 로그를 남기는 습관**이 중요하다. 흩어 놓으면 어디서부터 잘못됐는지 못 찾는다.

**둘째, 한도는 평균이 아니라 순간에 걸린다.** "월 50만 자"를 다 쓸 일이 없다고 안심하다가, 크롤러가 한 번 훑고 지나가면서 분당 한도에 걸려 서비스가 멈추는 식이다. 무료 API를 쓸 때 **호출부에 최소한의 큐나 간격 제어를 넣어 두는 것**이 사실상 필수다. 코드 몇 줄이면 되고, 이게 없으면 트래픽이 조금만 튀어도 전체가 흔들린다.

**셋째, 가장 위험한 건 "카드 등록형 무료"다.** 진짜 무료 API는 한도를 넘으면 에러를 준다. 최악의 경우 기능이 멈출 뿐이다. 반면 카드를 걸어 둔 종량제 API는 한도를 넘으면 **멈추지 않고 계속 과금된다.** 무한 루프 한 번, 재시도 로직의 버그 하나가 곧바로 청구서가 된다. 그래서 카드형 API를 붙일 때는 코드를 짜기 전에 콘솔에서 지출 상한부터 거는 순서가 맞다.

이 세 가지를 종합하면 실무 원칙이 하나 나온다. **무료 API는 "공짜인 유료 API"가 아니라 "언제든 성격이 바뀔 수 있는 외부 의존성"으로 취급하라.** 핵심 기능을 무료 API 하나에 통째로 걸어 두면, 그 API의 정책 변경이 곧 내 서비스의 장애가 된다. 대체 가능한 후보를 하나 정도는 알아 두고, 호출부를 교체하기 쉽게 감싸 두는 것만으로도 대부분의 사고를 흡수할 수 있다.
