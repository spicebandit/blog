---
title: "KIS API Python 연동 가이드 — 실전 함정과 해결법 [2편]"
...
pubDate: 2026-06-20T10:35:45+09:00
updatedDate: 2026-07-06T20:59:08+09:00
category: ax
...
조금 더 풀어 쓰면, 세 함정이 서로 다른 얼굴을 하고 있지만 뿌리는 같았다. **내 코드가 "요청을 보내면 답이 온다"고 믿었다는 것이다.**

토큰 1분 제한은 "발급받으면 성공한다"는 믿음이 깨진 경우다. 모의투자 서버의 500 응답은 "서버가 살아 있다"는 믿음이 깨진 경우고, SSL 문제는 "연결이 된다"는 가장 밑바닥 믿음이 깨진 경우다. 세 층위 모두에서 실패가 가능하다는 걸 코드가 알고 있어야 밤새 도는 프로그램이 된다.

여기서 실제로 도움이 된 습관이 하나 있다. **API를 부르는 코드를 한 겹 감싸서, 그 안에서만 재시도·타임아웃·로깅을 처리하는 것이다.** 분석 로직 쪽에서는 그냥 `get_price(code)`를 부르고, 실패하면 예외가 아니라 `None`을 받는다. 이렇게 나눠 두면 나중에 증권사를 바꾸거나 캐싱을 넣을 때 손댈 곳이 한 군데뿐이다. 반대로 분석 코드 곳곳에 `requests.get`이 흩어져 있으면, 재시도 로직을 넣는 순간 열 군데를 고쳐야 한다.

또 하나 배운 건 **실패를 조용히 삼키면 안 된다**는 점이다. 재시도 끝에 실패했는데 빈 값을 그냥 넘기면, 봇은 "데이터가 없다"와 "가져오지 못했다"를 구분하지 못한다. 전자는 정상적인 상황(거래 정지 종목 등)이고 후자는 장애다. 이 둘을 같게 처리하면, 어느 날 API가 종일 죽어 있었는데도 로그상으로는 아무 일 없었던 것처럼 보인다. 실패는 반드시 흔적을 남기게 해야 한다.

이 두 가지 — 호출부를 한 겹 감싸는 것, 실패에 흔적을 남기는 것 — 만 지켜도 초기 개발 속도는 거의 안 떨어지면서 운영 안정성은 크게 달라진다.

[다음 편에서는 드디어 봇의 "두뇌"를 만든다.](/blog/claude-code-stock-agent-3-analysts/) 분석가 3명의 프롬프트를 설계하고, LLM을 실제로 호출해 첫 분석 결과를 받아보는 과정이다.

## 따라 하기 — KIS API 키 발급 5단계

한국투자증권(KIS) 오픈 API를 직접 붙이려면 다음 순서로 준비한다.

1. **계좌 개설**: 한국투자증권 계좌를 만든다(비대면 앱). 자동매매 검증용 **모의투자 계좌도 함께 신청**한다.
2. **개발자 등록**: KIS Developers(apiportal.koreainvestment.com)에서 개발자 등록을 한다.
3. **앱키·앱시크릿 발급**: 앱을 등록하면 **APP KEY**와 **APP SECRET**이 나온다. 이 두 값이 인증의 핵심이다.
4. **계좌 정보 확인**: 계좌번호 앞 8자리(CANO)와 상품코드(ACNT_PRDT_CD, 보통 `01`)를 확인한다.
5. **환경변수(.env)에 저장**: 발급값을 코드에 직접 넣지 말고 `.env`로 관리한다.

```
KIS_ENV=vps          # 모의투자(vps) / 실전(prod)
KIS_APP_KEY=...
KIS_APP_SECRET=...
KIS_CANO=00000000
KIS_ACNT_PRDT_CD=01
...
**흔한 함정 (미리 알아두기)**
- 접근토큰은 발급 후 **재발급에 약 1분 제한**이 있다 → 토큰을 캐싱해 재사용해야 막히지 않는다.
- 모의투자 서버는 간헐적으로 500·타임아웃을 낸다 → **재시도 로직이 필수**다.
- 실전과 모의투자는 도메인·TR_ID가 다르다 → `KIS_ENV` 값으로 분기해 실수로 실계좌에 주문 나가는 걸 막는다.
