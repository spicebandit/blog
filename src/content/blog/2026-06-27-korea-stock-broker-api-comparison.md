---
title: "증권사 API 비교·추천 — 한투·키움·토스 자동매매 오픈API 총정리"
description: "증권사 API 비교·추천 가이드. 한국투자증권(KIS)·키움·토스증권·LS·대신 오픈API를 자동매매·시세조회 관점에서 비교하고, 목적별로 어떤 증권사 API가 좋은지 추천합니다. REST vs OCX·인증·모의투자·OS까지."
pubDate: 2026-06-27T19:13:11+09:00
updatedDate: 2026-07-06T20:20:14+09:00
category: ax
tags: ["증권사API", "증권사API비교", "증권사API추천", "자동매매"]
---

**증권사 API 비교·추천**을 한 번에 정리한 글입니다. 증권사 오픈 API는 "내 증권 계좌를 코드로 조종하는 공식 통로"로, 한 번 붙여 두면 시세를 실시간으로 받아 매수·매도 주문을 자동으로 내거나, 종목 화면을 띄우지 않고도 데이터를 긁어와 퀀트 분석·시세봇을 돌릴 수 있습니다. 그런데 같은 "증권사 API"라는 이름을 달고 있어도 한국투자증권·키움·토스증권·LS·대신은 연결 방식, 인증 절차, 돌아가는 운영체제(OS)가 전혀 다릅니다. 잘못 고르면 "리눅스 서버에 올리려다 윈도우 전용이라 포기"하는 일이 생깁니다. 이 글은 주요 **증권사 API를 자동매매와 시세조회 관점에서 비교**하고, **목적별로 어떤 증권사 API를 추천**하는지까지 짚는 실전 선택 가이드입니다.
...
## 1세대 OCX가 실제로 강제하는 것 — 표에 안 적히는 비용

비교표에서 "OS: 윈도우 필요"는 한 칸이지만, 실제로는 이 한 칸이 운영 방식 전체를 결정합니다. 1세대와 2세대 중 무엇을 고를지 고민 중이라면 이 대목이 사실상 결론을 내려 줍니다.

**1세대(OCX/COM)는 "사람이 로그인한 PC"를 전제로 만들어졌습니다.** HTS를 띄우고 로그인한 세션에 내 프로그램이 얹히는 구조라서, 그 PC가 꺼지면 봇도 멈춥니다. 그래서 실제로 돌리려면 다음 중 하나가 필요합니다.

- 집에 윈도우 PC를 24시간 켜 둔다 — 전기요금과 정전·업데이트 재부팅 리스크를 떠안습니다
- 윈도우 클라우드 인스턴스를 빌린다 — 리눅스보다 비쌉니다
- 노트북으로 장중에만 돌린다 — 자리를 비우면 전략이 멈춥니다

여기에 더해 세 가지가 따라옵니다. **32비트 파이썬에 묶이는 경우가 있고**(주요 데이터 분석 라이브러리 조합이 제한됩니다), **HTS가 업데이트되면 붙어 있던 프로그램이 깨질 수 있고**, **화면이 없는 환경에서 팝업이 뜨면 그대로 멈춥니다.** 마지막 항목이 특히 성가신데, 로그인 만료나 공지 팝업 하나에 봇이 조용히 대기 상태로 들어가 있는 일이 실제로 벌어집니다.

**2세대(REST/gRPC)는 이 전제가 없습니다.** HTTP로 토큰을 받아 호출하는 구조라 리눅스 서버든 라즈베리파이든 도커 컨테이너든 상관없습니다. 월 몇천 원짜리 리눅스 VM에 올려두고 잊어버릴 수 있고, 코드를 깃허브에 올려 CI로 배포하는 흔한 방식이 그대로 통합니다.

그래서 판단 기준은 이렇게 정리됩니다.

**이미 1세대 코드가 있고 잘 돌고 있다면 굳이 옮길 이유는 없습니다.** 자료가 많고 검증된 예제가 널려 있다는 건 진짜 장점입니다.

**하지만 지금 새로 시작한다면 2세대를 고르는 편이 거의 항상 낫습니다.** 이유는 기능 차이가 아니라 운영 비용입니다. 자동매매의 목적은 "내가 자는 동안에도 규칙대로 도는 것"인데, 1세대는 그 목적을 위해 PC 한 대를 계속 살려 두는 별도의 관리 업무를 만들어 냅니다. 전략을 다듬는 데 써야 할 시간이 인프라 관리로 새어 나갑니다.

키움이 2026년 REST·gRPC 기반 'Open API NEXT'를 내놓은 것도 이 흐름을 반영한 것으로 보입니다. 1세대가 나쁘다기보다, **개인 개발자의 실행 환경이 데스크톱에서 클라우드로 옮겨 간 것**입니다.
...
| 목적 | 추천 증권사 API | 이유 |
|------|----------------|------|
| **처음 시작·자료 많은 곳** | 한국투자증권(KIS) | 무료·레퍼런스 압도적, 막혀도 검색으로 해결 |
| **간편한 인증·빠른 시작** | 토스증권 | OAuth·AI 친화 문서, 국내/미국 주식 중심 |
| **파생·해외까지 넓게** | 한국투자증권(KIS) | 파생·해외주식·실시간 범위가 가장 넓음 |
| **클라우드 24시간 무인** | REST 2세대(KIS·토스·LS·키움 NEXT) | 리눅스 서버 상주 가능(1세대 OCX는 윈도우 필요) |
| **기존 HTS 사용자·풍부한 예제** | 키움증권 | 국내 사용자층·예제 방대(단 1세대는 윈도우) |

한 줄 결론: **입문·범용은 한국투자증권 KIS, 간편함 우선이면 토스증권**이 무난한 추천입니다. 아래에서 상황별로 조금 더 풀어 설명합니다.
