...
## 따라 하기 — LLM(Gemini) 분석가 연결

분석가 3명의 '두뇌'로 쓸 LLM을 붙이는 순서다.

1. **API 키 발급**: Google AI Studio에서 무료 **Gemini API 키**를 받는다.
2. **환경변수 추가**: `.env`에 `GEMINI_API_KEY=...`를 넣는다.
3. **프롬프트 배치**: 분석가 프롬프트(재무·기술·뉴스)는 `prompts/` 폴더에 원본으로 두고, 코드가 이를 불러 LLM에 전달한다. 프롬프트가 곧 각 분석가의 '직무기술서'다.
4. **1명부터 테스트**: 처음부터 3명을 돌리지 말고, **분석가 1명(예: 재무)만** 종목 하나로 호출해 응답이 스키마대로 나오는지 확인한 뒤 3명으로 확장한다.

**핵심 팁**: LLM 응답은 반드시 정해진 JSON 스키마(예: `schemas/analyst_output.schema.json`) 형식으로만 받도록 강제한다. 그리고 날짜·종목코드 같은 **사실값은 코드가 다시 덮어써** 환각을 차단한다 — 1편에서 강조한 "해석은 LLM, 사실·규칙은 코드" 원칙의 실제 적용이다.
