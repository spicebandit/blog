---
title: "Claude Code로 주식 자동매매 봇 만들기 — 설계와 아키텍처 [1편]"
...
updatedDate: 2026-07-06T20:57:15+09:00
category: ax
...
## 이 시리즈를 따라 하려면 — 준비물과 로드맵

직접 만들어보고 싶은 분을 위해, 시리즈 전체에 필요한 준비물과 순서를 먼저 정리한다.

**미리 챙기면 좋은 준비물**
- 증권사 계좌 + 오픈 API 신청: 한국투자증권(KIS) 기준. 실계좌와 함께 **모의투자 계좌**도 신청한다(검증용).
- 파이썬 3.x 설치
- LLM API 키: 분석가 두뇌용(예: Google AI Studio의 Gemini 무료 키)
- 텔레그램 봇: 폰으로 보고받고 승인하기 위한 용도(5편에서 상세)

**난이도·시간**: 코드를 조금 읽을 수 있으면 충분하다. 편당 30분~1시간, 전체는 주말 하루 정도면 첫 버전을 굴려볼 수 있다.

**시리즈 로드맵**
- 1편(이 글): 왜·어떤 구조로 만드는가 — 설계
- 2편: 증권사 API 연결(KIS) — 시세·잔고·주문의 토대
- 3편: LLM 분석가 3명 설계 — 판단의 두뇌
- 4편: 리스크·안전장치 — 잘못 사도 안전하게
- 5편: 자동 실행·텔레그램 — 무인 운영

**가장 중요한 원칙**: 처음부터 실계좌로 가지 말 것. 반드시 모의투자로 충분히 검증한 뒤 소액 실전으로 넘어간다.
