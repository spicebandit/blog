...
updatedDate: 2026-07-02T23:10:26+09:00
---
...
글을 더 읽기 전에 한 가지부터 정리하자. 2026년 현재 "AI 에이전트" 카테고리에는 비슷해 보이는 도구가 넘쳐나지만, 실제로는 **서로 다른 레이어를 커버**한다. 헤르메스 에이전트가 자주 비교되는 **Claude Code(클로드 코드)**, **OpenClaw(오픈클로)**, **Paperclip**과 어떻게 다른지 먼저 이해하면 나머지 내용이 훨씬 선명해진다.
...
- **OpenClaw(오픈클로)** — 자기 서버·PC에서 상시 실행하며 **왓츠앱·텔레그램 등 쓰던 메신저로 부리는** 오픈소스 개인 AI 비서. GitHub 스타 38만+의 2026년 최고 화제 오픈소스.
- **Paperclip** — 여러 AI 에이전트에게 태스크를 배정하고 팀처럼 관리하는 **에이전트 관리 SaaS 플랫폼**. 직접 코드를 실행하기보다 워크플로를 감독한다.

### 핵심 기능 비교표

| 기준 | Hermes Agent | Claude Code | OpenClaw | Paperclip |
|------|-------------|-------------|----------|-----------|
| **유형** | 단일 자율 에이전트 | 코딩 전문 에이전트 | 메신저 상주형 개인 비서 | 에이전트 관리 플랫폼 |
| **실행 환경** | 터미널·메신저·서버 상시 | 터미널·IDE (세션 기반) | 로컬·서버 데몬 상시 | 웹 보드 UI |
| **영속 메모리** | ✅ 디스크에 저장, 세션 간 유지 | ❌ 세션 종료 시 초기화 | ✅ 로컬 Markdown 파일로 유지 | ✅ 이슈·댓글·컨텍스트 저장 |
| **LLM 프로바이더** | 40+ 프로바이더, 300+ 모델 | Anthropic 전용 | 60+ 프로바이더 (Ollama 등 로컬 포함) | Anthropic Claude 기반 |
| **자가 학습·스킬 누적** | ✅ 스킬 자동 저장·재사용 | ❌ | ✅ 스킬 레지스트리(ClawHub) 생태계 | ✅ 에이전트별 메모리 |
| **메신저 게이트웨이** | ✅ Telegram·Discord·Slack 등 20+ | ❌ | ✅ WhatsApp·Telegram·Slack 등 21+ | ✅ (플러그인 방식) |
| **코딩 없이 사용 가능** | ✅ CLI·자연어 | ✅ 자연어 | ⚠️ 설치는 CLI, 사용은 채팅 | ✅ 보드 UI |
| **멀티 에이전트** | ⚡ 병렬 서브에이전트(실험적) | ❌ 단일 | ✅ 채널별 에이전트 라우팅 | ✅ 팀 구조 지원 |
| **오픈소스** | ✅ MIT 라이선스 | ❌ 상용 (Anthropic 구독) | ✅ MIT 라이선스 | ❌ 상용 SaaS |
| **셋업 난이도** | ★★★☆☆ 중간 | ★★☆☆☆ 낮음 | ★★★☆☆ 중간 | ★★☆☆☆ 낮음 |
| **주 대상** | 개발자·자동화 파워유저 | 개발자 (코딩 작업) | 상시 개인 비서를 원하는 개발자·파워유저 | 1인기업·소규모 팀 |
...
**OpenClaw가 맞는 경우:**
- 이미 쓰는 메신저(왓츠앱·텔레그램 등)를 그대로 AI 비서 창구로 만들고 싶을 때
- 대화 기록·메모리를 전부 내 기기에 두는 로컬 퍼스트가 중요할 때
- 60개 이상 프로바이더는 물론 Ollama 같은 로컬 모델까지 자유롭게 갈아끼우고 싶을 때
- 단, 셸 명령을 직접 실행하는 강력한 도구라 **커맨드라인을 이해하는 사용자에게만 권장**된다 — 프롬프트 인젝션 등 보안 리스크가 공론화된 만큼 권한·스킬 검증에 주의가 필요하다
...
네 도구는 경쟁이라기보다 **보완 관계**다. 실제로 Paperclip으로 태스크를 관리하면서 Hermes Agent가 실행·자동화를 담당하게 하거나, OpenClaw를 메신저 상주 비서로 두고 Claude Code로 코드 품질을 검토하는 조합도 가능하다. 다만 단 하나를 골라야 한다면: **서버에서 상시 실행되며, LLM 종속 없이, 메신저로 어디서나 접근 가능한 자율 에이전트**가 필요하다면 헤르메스가 2026년 현재 가장 명확한 선택지다.
